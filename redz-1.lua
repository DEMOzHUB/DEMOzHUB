...
Window:AddMinimizeButton({
  Button = {
    Image = "rbxassetid://130688450838044"  -- Troque pelo ID correto da imagem do botão
  },
  UICorner = {true, CornerRadius = UDim.new(0.5, 0)},
  UIStroke = {false, {}}
})

redzlib:SetTheme("Default")  -- Troque pelo nome do tema desejado
redzlib:SetTransparency(0)  -- Definindo transparência (0 a 1)

-- Criando uma aba
local DiscTab = Window:MakeTab({ Name = "Discord", Icon = "info"})
local HouseTab = Window:MakeTab({ Name = "House", Icon = "home"})
local CarTab = Window:MakeTab({ Name = "Vehicle", Icon = "rbxassetid://136049064787886"})
local MainTab = Window:MakeTab({ Name = "Troll", Icon = "bomb"})
local UserTab = Window:MakeTab({ Name = "Player", Icon = "user"})
local AvatarTab = Window:MakeTab({ Name = "Avatar", Icon = "shirt"})
local ToolsTab = Window:MakeTab({ Name = "Music All", Icon = "hammer"})
local MiscTab = Window:MakeTab({ Name = "Misc", Icon = "settings"})

DiscTab:AddDiscordInvite({
  DiscordTitle = "Demoz Hub | Community",
  DiscordIcon = "rbxassetid://130688450838044",  -- Troque pelo ID correto da imagem do ícone
  DiscordLink = "https://discord.gg/GywFaP9qEm"  -- Link do seu servidor Discord
})

local Paragraph = DiscTab:AddParagraph({ "Owner / Developer:", "Kaelix, R1ch" })
local Paragraph = DiscTab:AddParagraph({ "Credits:", "LolytaDev, VenomDev" })
...
LocalPlayer.CharacterAdded:Connect(function(character)
    if isSpectating then
        stopSpectating()
        pcall(function() SpectateToggleMainTab0:Set(false) end)
    end
end)

local valor_do_nome_do_joagdor

local DropdownPlayerUserTab = MainTab:AddDropdown({
    Name = "Select Target",
    Default = "",
    Multi = false,
...
function UptadePlayers()
    ...
    DropdownPlayerUserTab:Set(playerNames)
end

local Button = MainTab:AddButton({"Update List", function()
    UptadePlayers()
end})
...
local SpectateToggleMainTab0 = MainTab:AddToggle({
    Name = "View Player",
    Default = false,
    Callback = function(state)
        if state then
            if selectedPlayer then
                ...
                SpectateToggleMainTab0:Set(false)
            end
        else
...
Players.PlayerRemoving:Connect(function(player)
    updateDropdown(DropdownPlayerUserTab, SpectateToggleMainTab0)
...
local Section = MainTab:AddSection({"Kill/Bring"})

local DropdownKillPullMethod = MainTab:AddDropdown({
    Name = "Select Method (Kill/Bring)",
    Options = {"Sofá", "Ônibus"},
...
local Section = MainTab:AddSection({"Fling"})

local DropdownFlingMethod = MainTab:AddDropdown({
...
local flingToggle = MainTab:AddToggle({
    Name = "Fling Target",
    Default = false,
    Callback = function(state)
        if state then
...
local Section = AvatarTab:AddSection({"Avatar"})
...
local Button = AvatarTab:AddButton({"Update List", function()
    UptadePlayers()
end})
...
local Button = AvatarTab:AddButton({
    Name = "Copy Avatar < Selected Player >",
    Callback = function()
...
local Button = AvatarTab:AddButton({
    Name = "Copy Avatar < Nearest Player >",
    Callback = function()
        local Players = game:GetService("Players")
        local LP = Players.LocalPlayer
        local LChar = LP.Character
        ...
        local BodyColors = TPlayer.Character:FindFirstChild("Body Colors")
        if BodyColors then
            Remotes.ChangeBodyColor:FireServer(tostring(BodyColors.HeadColor))
        end
    end
})

local Section = AvatarTab:AddSection({"3D Avatar"})
...
-------------------------------------------------------------------------------------------------------------------------
local Section = AvatarTab:AddSection({"Avatar Editor"})
...
local TextBox = ToolsTab:AddTextBox({
    Name = "Inout ID for Sound All",
    Default = "",
    PlaceholderText = "Exemple: 6832470734",
...
local Toggle = ToolsTab:AddToggle({
    Name = "Loop",
    Default = false,
    Callback = function(state)
        loopAtivo = state
...
local function createSoundDropdown(title, musicOptions, defaultOption)
    ...
    local Dropdown = ToolsTab:AddDropdown({
        Name = title,
...
local Section = ToolsTab:AddSection({"Horror Sounds"})

-- Dropdown "Efeito/Terror"
createSoundDropdown("Select Horror Sounds", {
    ["efeito/terror"] = {
...
local Section = UserTab:AddSection({"Player Character"})
...
local TextBox = UserTab:AddTextBox({
    Name = "Nome do Jogador",
    PlaceholderText = "ex: lo → Player Name",
    Callback = function(Value)
...
local Button = UserTab:AddButton({"Ativar/Desativar headsit", function()
...
local Slider = UserTab:AddSlider({
    Name = "Speed Player",
    Increase = 1,
    MinValue = 16,
    MaxValue = 888,
...
local Slider = UserTab:AddSlider({
 ...
 local Button = UserTab:AddButton({
 ...
 local Toggle = UserTab:AddToggle({
    Name = "Infinite Jump",
    Default = false,
    Callback = function(Value)
...
local NoclipToggle = UserTab:AddToggle({
    Name = "Noclip",
    Default = false
})
...
local Section = UserTab:AddSection({"Spin"})
-- Serviços
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local LocalPlayer = Players.LocalPlayer
...
-- Variáveis
local Girando = false
local VelocidadeGiro = 360 -- graus por segundo
local SpinConnection

-- Função Toggle
local function ToggleGiro(state)
    Girando = state
    ...
    if Girando then
        SpinConnection = RunService.RenderStepped:Connect(function(dt)
            root.CFrame = root.CFrame * CFrame.Angles(0, math.rad(VelocidadeGiro) * dt, 0)
        end)
    else
...
local Section = UserTab:AddSection({"ESP"})
...
local function removeESP()
	for _, player in ipairs(Players:GetPlayers()) do
		if player.Character and player.Character:FindFirstChild(ESP_NAME) then
...
pcall(function()
    local Dropdown = HouseTab:AddDropdown({
        Name = "Select House",
...
pcall(function()
    local Button = HouseTab:AddButton({
...
pcall(function()
    local NoclipDoor = HouseTab:AddToggle({
...
pcall(function()
    local Toggle = HouseTab:AddToggle({
...
pcall(function()
    local Section = HouseTab:AddSection({"Teleports To Houses"})
end)
...
local casas = {
    ...
    ["Casa 36 ( Gamepass )"] = Vector3.new(249, 23, -2295),
    ["Casa 37 ( Gamepass )"] = Vector3.new(-1952, 29, 327)
}
...
pcall(function()
    local Section = HouseTab:AddSection({"Automatically Unban You"})
end)
...
-- Seção para Casa RGB
pcall(function()
    local Section = HouseTab:AddSection({"House RGB"})
end)
...
local Toggle = CarTab:AddToggle({
    Name = "View Car Camera ",
    Default = false,
    Callback = function(state)
        if state then
...
local Section = MiscTab:AddSection({"Scripts"})

local Button = MiscTab:AddButton({
    Name = "Fly <For Mobile>",
    Callback = function()
...
local Button = MiscTab:AddButton({
    Name = "Chat Spy",
    Callback = function()
        loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-chat-spy-81309"))()
    end
})
...
local Button = MiscTab:AddButton({
    Name = "infinite yield",
    Callback = function()
        loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-Infinite-Yield-mxrtnttzflw-82717"))()
    end
})
